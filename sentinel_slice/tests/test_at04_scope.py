...
thread_id owner (the substring before the first '/') does not match the
order principal. The structural, kitchen-blind scope check (step 4) rejects
with OUT_OF_SCOPE. One REJECTED receipt; chain verifies.
"""
...
def test_at04_scope(tmp_path):
    ...
    order = Order(
        ...
        role="account_manager",  # permitted
        capability_id="cap.email.draft_reply.v1",  # on-menu
        args={"thread_id": "user.dana/t9"},  # owner user.dana != principal
    ...
    assert rows[-1].reason_code == "OUT_OF_SCOPE"
